										...
										<tr>
											...
											<td>
												<div class="quantity clearfix">
													<input name="quantity" id="<?php echo $row['id'] ?>" class="form-control" type="number" value="<?php echo $value ?>" min="1" max="<?php echo $row['number']?>" onchange="onChangeSL(<?php echo $row['id'] ?>, <?php echo $row['number']?>)">
                                                    <div id="quantity_error<?php echo $row['id'] ?>" style="display: none;color: red">Số lượng bạn nhập đã vượt quá số lượng trong kho</div>
												</div>
											</td>
											<td>
												<span class="amount">
								...
								<table class="table total-price" style="border: 1px solid #ececec;">
									<tbody>
										...
										<tr>
											...
											<td colspan="2">
												<button type="button" onclick="redirect()" class="btn-next-checkout">Đặt hàng</button>
											</td>
										</tr>
	...
	<script>
        ...
        function redirect(){
            <?php if(!$this->session->userdata("sessionKhachHang")) {
                echo "window.location.href='dangnhap/dangnhap?isNeed=1'; return";
            }?>;
            window.location.href='info-order'
        }

		function onChangeSL(id, max){
			var sl = document.getElementById(id).value;
			var id_error = '#quantity_error' + id;
			if (sl > max) {
