...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            if(isset($_GET["isNeed"]) && $_GET["isNeed"] == "1") {
                $this->data['need']='Bạn vui lòng đăng nhập để mua hàng';
            }

            $this->data['title']='Kim Cương - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'sale.kim.cuong.2023@gmail.com';
            ...
            $this->email->from('sale.kim.cuong.2023@gmail.com', 'Kim Cương');
            $this->email->to($email);
            $this->email->subject('Kim Cương - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Kim Cương, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
                Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!!');
            $this->email->send();
            echo "<script>window.location.href = 'dangnhap/dangnhap?isRegister=1'</script>";
            return;
        }  
        $this->data['title']='Kim Cương - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->subject('Kim Cương - Lấy lại mật khẩu');
        ...
        $this->data['title']='Kim Cương - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Kim Cương - Cập nhật mật khẩu mới';
